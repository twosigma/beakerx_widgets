...
export class PlotFocus {
  ...
  static getDefault(
    ...
    focus.xspan = BigNumberUtils.minus(focus.xr, focus.xl);
  ...
  transformX(focus, value): void {
    this.transform(focus, value, 'xl', 'xr', 'xspan');
  }

  transformY(focus, value): void {
    this.transform(focus, value, 'yl', 'yr', 'yspan');
  }

  transformYRight(focus, valueR, valueY): void {
  ...
  transform(focus, value, left, right, span): void {
    if (focus[left] + value >= 0 && focus[right] + value <= 1) {
      this.transformBounds(focus, left, right, value);
      return;
    }
  ...
  transformBounds(focus, left, right, value): void {
    focus[left] += value;
    focus[right] += value;
  }

  transformLeftBound(focus, left, right, span): void {
    focus[left] = 0;
    focus[right] = focus[left] + focus[span];
  }

  transformRightBound(focus, left, right, span): void {
    focus[right] = 1;
    focus[left] = focus[right] - focus[span];
  }

  onModelFocusUpdate(newFocus: Focus): void {
    if (newFocus === null) {
      return;
    }
