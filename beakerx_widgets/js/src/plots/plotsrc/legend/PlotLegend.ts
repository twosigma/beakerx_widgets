...
export class PlotLegend {
  ...
  private adjustSvgWithLegendDimensions(svg: d3.Selection<SVGElement, any, HTMLElement, any>) {
    const svgWithLegendWidth = this.scope.plotSize.getPlotWithLegendWidth();
    const svgWithLegendHeight = this.scope.plotSize.getPlotWithLegendHeight();
  ...
  render() {
    ...
    for (const id in this.scope.legendMergedLines) {
  ...
  highlightElements(legendId, highlight) {
    ...
    for (let i = 0; i < elementsIds.length; i++) {
      const id = elementsIds[i];
      const data = this.scope.stdmodel.data[id];
